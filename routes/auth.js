...
router.post('/register', async (req, res) => {
    ...
    if (!username || !password) {
        return response(400, 'username & password harus diisi', '-', res)
    }
    //apabila akun sudah terdaftar
    db.query('SELECT * FROM users_1 WHERE username = ?', [username], (err, result) => {
        if (err) {
            return response(500, 'SERVER ERROR', '-', res)
        }
        return response(409, 'username sudah terdaftar', `${result[0].username} sudah terdaftar`, res)
    })
    //proses register
...
router.post('/login', (req, res) => {
    ...
    db.query(sql, [username], async (err, result) => {
        ...
        if (result.length === 0) {
            return response(404, 'username tidak terdaftar', '-', res)
        }
        const user = result[0]
        const passwordMatch = bcrypt.compare(password, user.password)
        if (!passwordMatch) {
            return response(400, 'password salah', '-', res)
        }
